module ActivityTracker # :nodoc:
  ...
  module ActMethods
    ...
    def acts_as_activity(actor = nil, options = {})
  ...
  module InstanceMethods
    ...
    def create_activity_from_self
      unless self.send( activity_options[:actor] ).blank?
    ...
    def create_action_user_activity(action, user)
      activity = Activity.new
      activity.item = self
      activity.action = action
      activity.user_id = user.id unless user.blank?
      activity.save
    end
    ...
    def track_activity_with_extra(action, extra)
      if activity_options[:actions].include?(action)
        activity = Activity.new
        activity.action = action.to_s
        activity.user_id = self.id
